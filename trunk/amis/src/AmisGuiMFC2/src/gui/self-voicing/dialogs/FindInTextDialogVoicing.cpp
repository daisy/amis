...
namespace amis
{
	namespace gui
	{
		namespace dialogs
		{
			FindInTextDialogVoicing::FindInTextDialogVoicing(amis::gui::dialogs::FindInTextDialog * dlg)
			{
				mpDialog = dlg;
			}

			FindInTextDialogVoicing::~FindInTextDialogVoicing(void)
			{
				mpDialog = NULL;
			}

			void FindInTextDialogVoicing::resolvePromptVariables(Prompt* pPrompt) 
			{
				...
				for (int i=0; i<pPrompt->getNumberOfItems(); i++)
				{
					if (pPrompt->getItem(i)->getPromptItemType() == PROMPT_VARIABLE)
					{
						...
						if (p_var->getName().compare("SELECTED_OR_DESELECTED") == 0)
						{
							...
							CButton* p_check_prev =	(CButton*)mpDialog->GetDlgItem(IDC_FINDPREVIOUS);
