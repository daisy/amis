...
Function .onInit
	...
	Error:
		IfSilent +2
		MessageBox MB_ICONSTOP "AMISの設定ディレクトリ $SETTINGS_DIR が見つかりません。"
		Abort
...
Function un.onInit
  IfSilent End
  MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2 "$(^Name) とそのコンポーネントをすべて削除しますか？" IDYES +2
  Abort
End:
FunctionEnd
...
Section Uninstall
	...
	Delete "$SMPROGRAMS\AMIS\ヘルプ (Japanese).lnk"
  	Delete "$SMPROGRAMS\AMIS\ショートカットキー (Japanese).lnk"
