...
Function .onInit
    ...
    ${ElseIf} ${AtLeastWinXP}
      ...
      ${AndIf} ${AtLeastServicePack} 2
		 ...
		 IfSilent DxCheck
         MessageBox MB_OK "Windows XP Service Pack 2が検知されました。新しい Service Pack がありますので、Windows Updateをして、最新の Service Pack をインストールしてください。"
         Goto DxCheck
    ...
    ${Else}
		  ...
		  IfSilent DxCheck
          MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2 "警告：OSがサポートされていません。AMISは正常に動作しないかもしれません。インストールを続行しますか？" IDYES DxCheck
...
DxCheck:
    ...
    ${If} $0 < 900
      ...
      IfSilent IeCheck
      MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2 "警告：AMISを使用するには、DirectX 9.0 以上が必要です。インストールを続ける前に、システムをアップデートすることをお勧めします。インストールを継続しますか？" IDYES IeCheck
...
IeCheck:
    ...
    ${If} $0 == 6.00
		...
		IfSilent End
        MessageBox MB_OK "Explorer 6が検知されました。最適な状態でAMISをお使いいただくには、バージョン７以降が推奨です。AMISのインストールが終了したら、Internet Explorerをアップデートしてください。"
    ${ElseIf} $0 < 6
        LogEx::Write "Warning user about IE version $0"
        IfSilent End
        MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2 "Internet Explorerのバージョンが古くて、AMISが動かない可能性があります。インストールを継続しますか？" IDYES End
...
Section -CopyLangpacks
    ...
    LogEx::Write "Copying default language pack files: jpn-JP"
...
Section -MSVCRuntime
    ...
    ${Else}
        ...
        ${If} $0 != "0"
			...
			IfSilent End
            MessageBox MB_ICONEXCLAMATION "Microsoft runtimeファイルのインストールでエラーが発生しました。AMISの使用中に問題が発生する場合は、テクニカルサポートにご連絡ください。"
        ${Else}
...
Section -JawsScripts
    ...
    ${If} $0 == 0
        ...
        MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON1 "AMIS用のJawsスクリプトをインストールしますか?" IDYES InstallScripts IDNO DoNotInstallScripts
    ...
    InstallScripts:
        ...
        ${If} $0 != "0"
			...
			IfSilent End
            MessageBox MB_ICONEXCLAMATION "AMIS用のJawsスクリプトのインストールでエラーが発生しました。次のページから、スクリプトのダウンロードとインストールをすることができます。http://daisy.org/amis"
        ${EndIf}
        Goto End
...
Section -JavaCheck
    ...
    ${If} $0 == 0
		...
		IncorrectVersion:
			...
			IfSilent End
        	MessageBox MB_OK "Java Runtimeのバージョンを、 1.6以上にアップグレードしてください。DAISY3のテキスト表示に必要です。AMISのインストール終了後に、次のページからJAVAをダウンロードしてインストールしてください。http://www.java.com "
        	Goto End
    ${Else}
...
Section -AdditionalIcons
    ...
    CreateShortCut "$SMPROGRAMS\AMIS\ヘルプ (Japanese).lnk" "$SETTINGS_DIR\lang\${LANGID}\help\${HELPFILE_NAME}"
    CreateShortCut "$SMPROGRAMS\AMIS\ショートカットキー (Japanese).lnk" "$SETTINGS_DIR\lang\${LANGID}\shortcuts\amiskeys.html"
SectionEnd
...
Function un.onInit
	IfSilent End
    MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2 " $(^Name) とそのコンポーネントをすべて削除しますか？" IDYES End
    Abort
End:
FunctionEnd
...
Function un.onUninstSuccess
    HideWindow
	IfSilent End
    MessageBox MB_ICONINFORMATION|MB_OK "$(^Name) を削除しました。"
End:
FunctionEnd
...
Section -un.RemoveHistoryBookmarks
	IfSilent Remove
    MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2 "ブックマークと履歴を削除しますか？" IDYES Remove IDNO End
Remove:
