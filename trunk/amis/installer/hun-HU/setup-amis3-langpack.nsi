...
OutFile "..\build\Setup-amis-langpack-${CUSTOM_LANG_NAME}.exe"
...
;**
Section -Post
  WriteUninstaller "$SETTINGS_DIR\lang\${CUSTOM_LANG_ID}\Uninstall-amis-langpack-${CUSTOM_LANG_NAME}.exe"
...
Function .onInit
	...
	Error:
		IfSilent +2
		MessageBox MB_ICONSTOP "Az AMIS beállításait tartalmazó $SETTINGS_DIR mappa nem található."
		Abort
...
Function un.onInit
  IfSilent End
  MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2 "Biztosan eltávolítja az $(^Name) programot annak minden összetevőjével együtt?" IDYES +2
  Abort
End:
FunctionEnd
...
Function un.onUninstSuccess
  HideWindow
  IfSilent End
  MessageBox MB_ICONINFORMATION|MB_OK "Az $(^Name) eltávolítása a számítógépről sikeresen befejeződött."
End:
FunctionEnd
...
Section Uninstall
	...
	Delete "$SMPROGRAMS\AMIS\Súgó (${CUSTOM_LANG_NAME}).lnk"
  	Delete "$SMPROGRAMS\AMIS\Billentyűparancsok (${CUSTOM_LANG_NAME}).lnk"
