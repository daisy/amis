...
Function .onInit
    ...
    ${ElseIf} ${AtLeastWinXP}
      ...
      ${AndIf} ${AtLeastServicePack} 2
         ...
         MessageBox MB_OK "Az ön számítógépére a Windows XP Service Pack 2 van telepítve.  A Szervízcsomag újabb verziója elérhetõ a Windows Update webhelyen. Azt ajánljuk, hogy telepítse rendszerére az elérhetõ frissítéseket."
         Goto DxCheck
...
DxCheck:
    ...
    ${If} $0 < 900
        MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2 "Figyelmeztetés: Az AMIS A DirectX 9.0 vagy magasabb verzióját igényli.  Folytathatja most a telepítést, azonban a telepítés elõtt ajánlott a rendszer frissítése.  Folytatja?" IDYES IeCheck
...
IeCheck:
    ...
    ${If} $0 == 6.00
        ...
        MessageBox MB_OK "Ön az Internet Explorer 6-os verziójával rendelkezik.  A 7-es verzió vagy magasabb szükséges az optimális teljesítmény eléréséhez.  Az AMIS telepítése után azt ajánljuk, hogy frissítse a rendszerén futó Internet Explorer böngészõt."
    ${ElseIf} $0 < 6
...
Section -MSVCRuntime
    ...
    ${Else}
        ...
        ${If} $0 != "0"
            ...
            MessageBox MB_ICONEXCLAMATION 'Hiba történt A Microsoft futásidejû környezet telepítése közben.  Ha problémát észlel az AMIS futtatása közben, vegye fel a kapcsolatot az "Informatika a látássérültekért" Alapítvány ügyfélszolgálatával!'
        ${Else}
...
Section -JavaCheck
    ...
    ${If} $0 == 0
        ...
        ${If} $R0 < 1.6
            ...
            MessageBox MB_OK "A Java Futtatókörnyezet frissítése az 1.6 vagy magasabb verzióra történõ frissítése szükséges a DAISY 3 könyvek szövegének teljeskörû megjelenítéséhez. Az AMIS telepítése után Bármikor letöltheti a futtatókörnyezetet a http://www.java.com oldalról, majd telepítheti azt."
        ${Else}
            LogEx::Write "Correct Java version installed ($R0)"
        ${EndIf}
    ${Else}
        LogEx::Write "Java not found"
        MessageBox MB_OK "A Java Futtatókörnyezet 1.6 vagy magasabb verziójának telepítése szükséges a DAISY 3 könyvek szövegének teljeskörû megjelenítéséhez. Az AMIS telepítése után Bármikor letöltheti a futtatókörnyezetet a http://www.java.com oldalról, majd telepítheti azt."
    ${EndIf}
...
;**
Function un.onInit
    MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2 "Biztos, hogy el kívánja távolítani az $(^Name) programot minden összetevõjével együtt?" IDYES +2
    Abort
FunctionEnd
...
Section -un.RemoveHistoryBookmarks
    MessageBox MB_ICONQUESTION|MB_YESNO|MB_DEFBUTTON2 "El kívánja távolítani a könyvjelzõket és a legutoljára megnyitott könyvek listáját?" IDYES Remove IDNO End
Remove:
